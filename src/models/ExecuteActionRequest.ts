/* tslint:disable */
/* eslint-disable */
/**
 * SpatioAPI
 * The REST API that owns every resource in your Spatio workspace — notes, sheets, slides, tasks, calendar events, mail, chat, files, and contacts. SpatioMCP wraps this API; Spatio Desktop reads from it. You can call it directly from your own code.  All requests must be authenticated with a Personal Access Token (`Authorization: Bearer pat_...`) and use HTTPS.  This specification is generated from the platform-service Go source on every push to `main`. The spec — not hand-written documentation — is the source of truth: server stubs and SDKs are generated from it, and any drift between the spec and the running service fails CI. 
 *
 * The version of the OpenAPI document: v1
 * Contact: hello@spatio.app
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * Generic action-execute payload. Tool-specific shape lives in `params`.
 * @export
 * @interface ExecuteActionRequest
 */
export interface ExecuteActionRequest {
    /**
     * 
     * @type {string}
     * @memberof ExecuteActionRequest
     */
    actionId: string;
    /**
     * 
     * @type {{ [key: string]: any; }}
     * @memberof ExecuteActionRequest
     */
    params?: { [key: string]: any; };
    /**
     * 
     * @type {string}
     * @memberof ExecuteActionRequest
     */
    accountId?: string;
}

/**
 * Check if a given object implements the ExecuteActionRequest interface.
 */
export function instanceOfExecuteActionRequest(value: object): value is ExecuteActionRequest {
    if (!('actionId' in value) || value['actionId'] === undefined) return false;
    return true;
}

export function ExecuteActionRequestFromJSON(json: any): ExecuteActionRequest {
    return ExecuteActionRequestFromJSONTyped(json, false);
}

export function ExecuteActionRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): ExecuteActionRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'actionId': json['actionId'],
        'params': json['params'] == null ? undefined : json['params'],
        'accountId': json['accountId'] == null ? undefined : json['accountId'],
    };
}

export function ExecuteActionRequestToJSON(json: any): ExecuteActionRequest {
    return ExecuteActionRequestToJSONTyped(json, false);
}

export function ExecuteActionRequestToJSONTyped(value?: ExecuteActionRequest | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'actionId': value['actionId'],
        'params': value['params'],
        'accountId': value['accountId'],
    };
}

